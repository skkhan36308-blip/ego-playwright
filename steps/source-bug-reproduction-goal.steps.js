...
Given('the user opens the application', async function () {
  this.pageObject = new SourceBugReproductionGoalCreatePage(this.page);
  await this.pageObject.goto();
});
When('the user clicks "Username"', async function () {
  await this.pageObject.clickUsername();
});

When('the user fills "Username" with "admin"', async function () {
  await this.pageObject.fillUsername2("admin");
});
...
When('the user clicks "Login"', async function () {
  await this.pageObject.clickLogin();
});
