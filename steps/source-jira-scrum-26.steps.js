...
Given('the user opens the application', async function () {
  this.pageObject = new SourceJiraScrum26RequirementPage(this.page);
  await this.pageObject.goto();
});
When('the user fills "Username" with "sana"', async function () {
  await this.pageObject.fillUsername("sana");
});
